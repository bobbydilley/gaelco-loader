...
static gaelco_config_t cfg = {
    .fullscreen = 0,
    /* 0 = "follow the game": the window is created at whatever size the
     * game asks to render at, so the letterbox scaler stays disabled and
     * behaviour matches a bare passthrough. Set width/height in gaelco.ini
     * to force a different window size and turn scaling on. */
    .width = 0,
    .height = 0,
    .render_width = 640,
    .render_height = 480,
...
void config_load(void)
{
    ...
    /* width/height == 0 means "follow the game"; only clamp real overrides. */
    if (cfg.width != 0 && cfg.width < 64)
    {
        cfg.width = 64;
    }

    if (cfg.height != 0 && cfg.height < 64)
    {
        cfg.height = 64;
    }
